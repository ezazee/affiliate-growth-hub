...
export async function GET() {
  try {
    ...
    if (productsWithoutCommission.length > 0) {
      insights.push(`⚠️ ${productsWithoutCommission.length} produk tidak memiliki commissionType atau commissionValue`);
    }
    
    if (ordersWithoutCommissions.length > 0) {
      insights.push(`❌ ${ordersWithoutCommissions.length} order berstatus paid tidak memiliki komisi yang dibuat`);
    }
    
    if (ordersWithoutAffiliator.length > 0) {
      insights.push(`⚠️ ${ordersWithoutAffiliator.length} order berstatus paid tidak memiliki affiliatorId`);
    }
    
    if (commissions.length === 0) {
      insights.push(`📊 Tidak ada data komisi sama sekali`);
    }
